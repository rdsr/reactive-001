...
class NodeScalaSuite extends FunSuite {
  ...
  test("Test all with exception") {
    val fs = List(Future { 1 }, Future { 2 }, Future { throw new Exception("This future fails") })
    val f = Future.all(fs)

    try {
      Await.result(f, 10 seconds)
    } catch {
      case e: Exception ⇒ assert(e.getMessage().equals("This future fails"))
    }
  }
